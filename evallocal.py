...
if __name__ == '__main__':
    ...
    input_array = np.zeros((input_data.shape[0], input_data.shape[1] + edges, input_data.shape[2] + edges))
    input_array[0] = -1
    input_array[:,edge:-edge,edge:-edge] = input_data
    for i in range(output_data.shape[0]):
        for j in range(output_data.shape[1]):
            target = output_data[i,j]
            local_input_array = input_array[:,i:i+input_shape,j:j+input_shape]
            pred[i,j] = neural_network(local_input_array.flatten(), W1, b1, W2, b2)
